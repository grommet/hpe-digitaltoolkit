...
const WatchIcon = require('grommet/components/icons/base/Watch');
const Marquee = require('../modules/MarqueeCard');
...
const Primary = React.createClass({
  ...
  _renderNewsFeed: function () {
    ...
    return (
      <Box colorIndex="light-2" pad="large" align="center">
        <Box full="horizontal">
          <Heading tag="h2" strong={true}>
            Recent News
          </Heading>
          <Columns size="medium" masonry={true} maxCount={7} responsive={true}>
            {blogPostCard}
            {featuredPostCard}
            {socialFeedCard1}
            {socialFeedCard2}
    ...
    return (
      <div>
        ...
        <Marquee darkTheme={false}
          ...
          label="label"
          heading={<Headline size="medium" margin="small" strong={true}>
            Accelerate your transformation with the cloud
          </Headline>}
          subHeading="HPE can help you benefit now from your right mix of cloud"
        ...
        <Box pad={{horizontal: 'large'}}><p><strong>Section</strong></p></Box>
        {this._loremIpsum()}
      </div>
    );
  }
